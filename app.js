...
onAuthStateChanged(auth, async user => {
  ...
  if (user && !user.isAnonymous) {
    ...
    state.workoutsUnsub = onSnapshot(
      ...
      snap => {
        ...
        if (snap.empty) {
          // Ensure the user doc exists with isPremium: false before seeding
  ...
  if (state._stripeSuccessReturn && user && !user.isAnonymous) {
    ...
    if (state.premiumStatus) {
      setTimeout(() => alert('Paiement réussi ! Bienvenue dans le Premium.'), 300);
    } else {
      setTimeout(async () => {
        await loadPremiumStatus(user);
        renderGuidedPrograms();
        alert('Paiement réussi ! Bienvenue dans le Premium.');
      }, 3000);
    }
  }
});
