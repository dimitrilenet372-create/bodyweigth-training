...
function renderGuidedPrograms() {
  document.getElementById('guided-list').innerHTML = GUIDED_PROGRAMS.map(p => `
    <div class="guided-card" onclick="openGuidedProgram('${p.id}')">
      ...
      ${isPremium() ? '' : `<div class="guided-lock-icon"><svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg></div>`}
    </div>
  `).join('');
}
...
function getMuscleImg(muscle) {
  const maps = {
    ...
    'Cardio':          bodyFront({quads:1, shoulders:1, abs:1}),
    'Tous':            bodyFront({pecs:1, shoulders:1, abs:1, quads:1}),
  };
