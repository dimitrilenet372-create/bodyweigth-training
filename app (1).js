...
function renderChips() {
  ...
  container.innerHTML = chipOrder.map(val => {
    ...
    return `<div class="muscle-chip${active?' active':''}${val==='Tous'?' chip-all':''}" data-muscle="${val}">
      <div class="chip-img-wrap">
        ${img
          ? `<img src="${img}" alt="${val}" loading="lazy" onerror="this.style.display='none';this.nextSibling.style.display='flex'">
  ...
  // Re-bind click & pointer events after re-render
  document.querySelectorAll('.muscle-chip').forEach(chip => {
    chip.addEventListener('click', () => {
      if (chipDragging) return; // ignore click during drag
...
function initChipPointerDrag() {
  ...
  container.addEventListener('pointermove', e => {
    ...
    if (!moved && (Math.abs(dx) > 6 || Math.abs(dy) > 6)) {
      moved = true;
      chipDragging = pointerDownChip.dataset.muscle;
      pointerDownChip.classList.add('dragging-chip');
    }
    if (!moved) return;
    // Find chip under pointer
    ...
    if (target && target !== pointerDownChip) {
      ...
      if (from >= 0 && to >= 0 && from !== to) {
        ...
        // Move DOM nodes directly (no re-render = no flicker)
        if (from < to) container.insertBefore(pointerDownChip, target.nextSibling);
        else           container.insertBefore(pointerDownChip, target);
      }
    }
  }, { passive: true });
